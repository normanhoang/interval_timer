...
- `lib/colors.ts` — soft interval palette constants (`INTERVAL_COLORS`, `WORK_COLOR`, `REST_COLOR`, `INK`, `PRIMARY`) plus `isLightColor()` for picking readable foregrounds on a swatch and `LEGACY_COLOR_MAP` for retired palettes. **Keep in sync with `tailwind.config.js` theme tokens** (tailwind config can't import TS, so hexes are duplicated deliberately).
...
- `lib/WorkoutsContext.tsx` — `workouts` CRUD + `reorderWorkouts` + `sessions` (`addSession`/`deleteSession`/`clearSessions`), persisted to AsyncStorage keys `@hiit/workouts` / `@hiit/sessions`, `hydrated` flag, seeds two example workouts on first launch, and rewrites interval colors from retired palettes via `LEGACY_COLOR_MAP` on hydrate. Consumed via `useWorkouts()`.
...
- **Non-className colors** (Ionicons `color`, gradients, blur tint, svg strokes, placeholderTextColor) come from `useThemeColors()` in `lib/theme.ts` — never hardcode `INK`/rgba inks in components. Accent `primary` (#A78BFA) and the interval palette (`lib/colors.ts`) are shared by both themes.
...
- `tailwind.config.js` — includes `nativewind/preset`, sets content paths, and defines the `ink`/`primary`/`interval-*` color palette
